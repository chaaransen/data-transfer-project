...
public class ICPSRDataTransferClient {
  ...
  private final List<String> ICPSR_SERVER_REGIONS =
  ...
  private final ICPSRS3ClientFactory ICPSRS3ClientFactory;
  private final Monitor monitor;
  private S3Client s3Client;
  private String bucketName;

  public ICPSRDataTransferClient(
          Monitor monitor,
          ICPSRS3ClientFactory ICPSRS3ClientFactory,
          long sizeThresholdForMultipartUpload,
          long partSizeForMultiPartUpload) {
    this.monitor = monitor;
    this.ICPSRS3ClientFactory = ICPSRS3ClientFactory;
  ...
  public void init(String keyId, String applicationKey, String exportService)
    ...
    for (String region : ICPSR_SERVER_REGIONS) {
      try {
        s3Client = ICPSRS3ClientFactory.createS3Client(keyId, applicationKey, region);
    ...
    if (listBucketsResponse == null || userRegion == null) {
      throw new ICPSRCredentialsException(
  ...
  public String uploadFile(String fileKey, File file) throws IOException {
    if (s3Client == null || bucketName == null) {
      throw new IllegalStateException("ICPSRDataTransferClient has not been initialised");
    }
