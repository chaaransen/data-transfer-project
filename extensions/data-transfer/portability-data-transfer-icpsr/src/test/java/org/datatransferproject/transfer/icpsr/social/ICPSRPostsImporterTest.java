...
package org.datatransferproject.transfer.icpsr.social;
...
public class ICPSRPostsImporterTest {
  ...
  public void testImportSingleActivity() throws Exception {
    ...
    ICPSRPostsImporter icpsrPostsImporter =
        new ICPSRPostsImporter(
            monitor, client, new ObjectMapper(), "http://example.com", "export-service");
    icpsrPostsImporter.importItem(UUID.randomUUID(), executor, authData, resource);
    ...
    assertEquals(
        "ICPSRPostsImporter changed the order of fields in the body.", "content", actual.name(2));
